...
from app.dependencies import get_check_service, get_metrics_service
...
async def list_checks(
    ...
    return await checks.list_checks(monitor_id, from_dt=from_, to_dt=to, limit=limit)
...
async def get_metrics(
    ...
    return await metrics.get_metrics(monitor_id, period)
...
async def get_uptime(
    ...
    return await metrics.get_uptime(monitor_id, period)


@dashboard_router.get("/summary")
async def get_dashboard_summary(metrics: MetricsService = Depends(get_metrics_service)) -> dict:
    """Real, dynamically-computed dashboard totals (section 27) -- never
    hardcoded sample numbers."""
    return await metrics.get_dashboard_summary()
